...
import { handleUpdateRoleBasic } from '../../services/api';

interface IProps {
  actionRef?: ActionType;
  visible: boolean;
  setVisible: (e: boolean) => void;
  roleUpdateBasicInfo: Role.RoleUpdateBasic;
}
...
const UpdateRoleForm: React.FC<IProps> = ({
  ...
  return (
    <ModalForm<Role.RoleUpdateBasic>
      title={'编辑角色'}
      open={visible}
      width="400px"
      submitter={{
      ...
      onFinish={async (values: Role.RoleUpdateBasic) => {
        const success = await handleUpdateRoleBasic(values);
        if (success) {
          // 关闭弹窗
          setVisible(false);
          // 刷新列表
